...
counter = 1
for snap in range(flag, frames + 1, flag):
        ...
        with open(f"{molecule_name}_MD.arc", "r") as inp:
            for i, line in enumerate(inp): #sed command in bash
                if i >= begins and i <= ends:
                    ...
                    break
    # with open(f"{molecule_name}_MD.arc") as f:
    #     lines = f.readlines()
    # with open(f"cmpd_{counter}", "w") as f:
    #     f.writelines(lines[begins:ends])
    counter += 1
        #A_glucosecounter += 1
print()
...
for index in range(1, snapshots+1):
    ...
    for a in range(1,len(solvent_number)):
        ...
        if (solvent_Z[a]-avg_Z) > box_Z/2:
            ...
            solvent_Z[a] += box_Z
    #solvent_X[solvent_X - avg_X > box_X / 2] -= box_X
    #solvent_X[solvent_X - avg_X < -box_X / 2] += box_X
    #solvent_Y[solvent_Y - avg_Y > box_Y / 2] -= box_Y
    #solvent_Y[solvent_Y - avg_Y < -box_Y / 2] += box_Y
    #solvent_Z[solvent_Z - avg_Z > box_Z / 2] -= box_Z
    #solvent_Z[solvent_Z - avg_Z < -box_Z / 2] += box_Z
    ...
    while not solvent_complete:
        ...
        for p in range(1, len(final_number) + 1):
            ln_num = final_number[p-1]
            with open(f"cmpd_{index}") as f:
                ...
                if columns == 6:
                    ...
                    connect_1.append(int(line.split()[6]))
                    connect_2.append(0)
                    connect_3.append(0)
                    connect_4.append(0)
                elif columns == 8:
                    connect_1.append(int(line.split()[6]))
                    connect_2.append(int(line.split()[7]))
                    connect_3.append(0)
                    connect_4.append(0)
                elif columns == 9:
                    connect_1.append(int(line.split()[6]))
                    connect_2.append(int(line.split()[7]))
                    connect_3.append(int(line.split()[8]))
                    connect_4.append(0)
                elif columns == 10:
                    connect_1.append(int(line.split()[6]))
                    connect_2.append(int(line.split()[7]))
                    connect_3.append(int(line.split()[8]))
                    connect_4.append(int(line.split()[9]))
                col.append(columns)
